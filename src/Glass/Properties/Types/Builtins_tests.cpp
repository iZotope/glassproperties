...
TEST(PointSerialization, PointInBrackets) {
	auto deserialized = PointPropertyType::deserialize("[2.0, 3.0]");
	ASSERT_FALSE(deserialized);
}

TEST(PointSerialization, PointIn) {
	auto deserialized = PointPropertyType::deserialize(" 2.0 3.0 ");
	ASSERT_TRUE(std::nullopt != deserialized);
	auto value = Point{2.f, 3.f};
	ASSERT_EQ(*deserialized, value);
}

TEST(PointSerialization, PointInComma) {
	auto deserialized = PointPropertyType::deserialize(" 2.0, 3.0 ");
	ASSERT_TRUE(std::nullopt != deserialized);
	auto value = Point{2.f, 3.f};
	ASSERT_EQ(*deserialized, value);
...
TEST(PointSerialization, SerializeDeserialize) {
	auto value = Point{4.f, 4.5f};
	auto serialized = PointPropertyType::serialize(value);
	ASSERT_EQ(serialized, "4 4.5");
	auto deserialized = PointPropertyType::deserialize(serialized);
	ASSERT_TRUE(std::nullopt != deserialized);
	ASSERT_EQ(value, *deserialized);
}

TEST(SizeSerialization, SizeInBrackets) {
	auto deserialized = SizePropertyType::deserialize("[2.0, 3.0]");
	ASSERT_FALSE(deserialized);
}

TEST(SizeSerialization, TwoFewValues) {
	auto deserialized = SizePropertyType::deserialize("2.0");
	ASSERT_FALSE(deserialized);
}

TEST(SizeSerialization, TooManyValues) {
	auto deserialized = SizePropertyType::deserialize("2.0 3.0 4.0");
	ASSERT_FALSE(deserialized);
}

TEST(SizeSerialization, SizeIn) {
	auto deserialized = SizePropertyType::deserialize(" 2.0 3.0 ");
	ASSERT_TRUE(std::nullopt != deserialized);
	auto value = Glass::Size{2.f, 3.f};
	ASSERT_EQ(*deserialized, value);
}

TEST(SizeSerialization, SizeInComma) {
	auto deserialized = SizePropertyType::deserialize(" 2.0, 3.0 ");
	ASSERT_TRUE(std::nullopt != deserialized);
	auto value = Glass::Size{2.f, 3.f};
	ASSERT_EQ(*deserialized, value);
...
TEST(SizeSerialization, SizeDeserialize) {
	auto value = Glass::Size{4.f, 4.5f};
	auto serialized = SizePropertyType::serialize(value);
	ASSERT_EQ(serialized, "4 4.5");
	auto deserialized = SizePropertyType::deserialize(serialized);
	ASSERT_TRUE(deserialized);
	ASSERT_EQ(*deserialized, value);
}

TEST(RectSerialization, RectInBrackets) {
	auto deserialized = RectPropertyType::deserialize("[-10, -2.9, 3.0, 4]");
	ASSERT_FALSE(deserialized);
}

TEST(RectSerialization, TooFewValues) {
	auto deserialized = RectPropertyType::deserialize("-10 -2.9 3.0");
	ASSERT_FALSE(deserialized);
}

TEST(RectSerialization, TooManyValues) {
	auto deserialized = RectPropertyType::deserialize("-10 -2.9 3.0 4 7");
	ASSERT_FALSE(deserialized);
}

TEST(RectSerialization, RectIn) {
	auto deserialized = RectPropertyType::deserialize(" 1 2.0 3.0 4 ");
	ASSERT_TRUE(std::nullopt != deserialized);
	auto value = Glass::Rect{Point{1, 2.f}, Glass::Size{3.f, 4}};
	ASSERT_EQ(*deserialized, value);
}

TEST(RectSerialization, RectInCommas) {
	auto deserialized = RectPropertyType::deserialize(" 1, 2.0 , 3.0, 4 ");
	ASSERT_TRUE(deserialized);
	auto value = Glass::Rect{Point{1, 2.f}, Glass::Size{3.f, 4}};
	ASSERT_EQ(*deserialized, value);
...
TEST(RectSerialization, UnnormalizedRectIn) {
	auto deserialized = RectPropertyType::deserialize("-10 -2.9 -3 -4");
	ASSERT_TRUE(deserialized);
	ASSERT_EQ(*deserialized, (Glass::Rect{{-10, -2.9f}, {-3, -4}}));
}

TEST(RectSerialization, SerializeDeserialize) {
	auto value = Glass::Rect{Point{1.9f, 2.f}, Glass::Size{0.8f, 4.2f}};
	auto serialized = RectPropertyType::serialize(value);
	auto deserialized = RectPropertyType::deserialize(serialized);
	ASSERT_TRUE(std::nullopt != deserialized);
	ASSERT_EQ(*deserialized, value);
}
